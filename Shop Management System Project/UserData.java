...
public class UserData extends JFrame {
    ...
    private String[] column = { "User Name", "Password", "Phone" };
    ...
    UserData() {
        ...
        try {
            ...
            for (int i = 2; i < totalLines; i++) {
                ...
                String x = line.substring(0, 3);
                if (x.equals("User")) {
                    rows[0] = Files.readAllLines(Paths.get(file)).get(i).substring(12); // User Name
                    rows[1] = Files.readAllLines(Paths.get(file)).get((i + 1)).substring(11); // phone
                    rows[2] = Files.readAllLines(Paths.get(file)).get((i + 2)).substring(8); // pass
                    model.addRow(rows);
                }
            }
